...
export const StrategiesList: React.FC<StrategiesListProps> = ({
  ...
  const formatTime = (dateStr: string) => {
    try {
      // Custom implementation instead of using date-fns
            ...
            <div className="p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="relative">
                  {/* Using conditional rendering to ensure proper ARIA attribute values */}
                  {actionMenuOpen === strategy.id ? (
                    <button
                      onClick={() => setActionMenuOpen(null)}
                      className="p-2 text-gray-400 hover:text-gray-300 rounded-full hover:bg-gray-700"
                      aria-label={`Close strategy actions for ${strategy.name}`}
                      aria-expanded="true"
                      aria-haspopup="true"
                    >
                    ...
                    <button
                      onClick={() => setActionMenuOpen(strategy.id)}
                      className="p-2 text-gray-400 hover:text-gray-300 rounded-full hover:bg-gray-700"
                      aria-label={`Open strategy actions for ${strategy.name}`}
                      aria-expanded="false"
                      aria-haspopup="true"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
